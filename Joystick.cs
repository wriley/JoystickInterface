...
namespace JoystickInterface
{
    ...
    public class Joystick
    {
        ...
        public int WaitForButton(int waitSeconds)
        {
            byte[] state1 = joystickDevice.CurrentJoystickState.GetButtons();
            byte[] state2 = new byte[joystickDevice.CurrentJoystickState.GetButtons().Length];
            int i = 0;
            ...
            while (notfound && (DateTime.Now.Subtract(startTime).Seconds < waitSeconds))
            {
                state2 = joystickDevice.CurrentJoystickState.GetButtons();

                i = 0;
                foreach (byte t in state1)
                {
                    if (!t.Equals(state2[i]))
                    {
                        notfound = false;
                        break;
                    }
